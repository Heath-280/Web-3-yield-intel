export async function analyzeYieldWithAI(input: {
  protocol: string;
  pool: string;
  apy: number;
  apyChange: number;
...
EXPLANATION:
The current APY of ${input.apy}% comes from activity in the ${
    input.protocol
  } ${input.pool} pool. The recent change of ${
    input.apyChange
  }% reflects short-term liquidity dynamics rather than long-term instability.
